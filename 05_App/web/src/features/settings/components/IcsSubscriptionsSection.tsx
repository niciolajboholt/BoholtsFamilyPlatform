...
// idrætskalender osv.) — skrivebeskyttet. Samme dialog-mønster som
// ShareLinkDialog/CalendarConnectionsSection.
export function IcsSubscriptionsSection() {
  ...
  useEffect(() => {
    ...
    // Effekten genkører hver gang dialogen genåbnes (ikke kun ved mount) —
    // uden dette ville et andet besøg i dialogen vise de forrige data et
    // øjeblik, før den friske hentning er færdig. Samme accepterede
  ...
  return (
    <>
      <SettingsSectionHeader>Delte kalendere</SettingsSectionHeader>

      <Card>
        <CardContent sx={{ p: 3 }}>
          <SettingsLinkRow
            icon={<LinkRounded color="action" />}
            title="Delte kalendere (ICS)"
            subtitle={
              subscriptions.length > 0
                ? `${subscriptions.length} af ${maxSubscriptions} tilføjet`
                : "Fx en skole- eller idrætskalender"
            }
      ...
      <Dialog open={isDialogOpen} onClose={() => setIsDialogOpen(false)} fullWidth maxWidth="xs">
          ...
          {isLoading ? (
            ...
            <>
              <Typography color="text.secondary" sx={{ mb: 2 }}>
                Tilføj en delt kalender via dens ICS-link — skrivebeskyttet,
                ingen login nødvendig til den konto.
              </Typography>
              ...
              {atCap ? (
                <Alert severity="info">
                  Højst {maxSubscriptions} delte kalendere ad gangen. Fjern en
        ...
        <DialogActions sx={{ px: 3, pb: 2.5 }}>
          <Button onClick={() => setIsDialogOpen(false)}>Luk</Button>
        </DialogActions>
      </Dialog>
    </>
  );
